...
required_helpers = [
    ...
    "def _smx_openai_extension_video_id(",
]
missing = [name for name in required_helpers if name not in content]
if missing:
    raise SystemExit("Missing required helper(s): " + ", ".join(missing))
...
func = content[start:content.find("\ndef _poll_google_video_operation", start)]
if "elif create is not None:\n    elif create is not None:" in func:
    raise SystemExit("Duplicate elif remains.")
if "job = extend(" in func or "job = remix(" in func:
    raise SystemExit("Old SDK extension call remains.")
if "_smx_openai_extend_video_via_json_endpoint(" not in func:
    raise SystemExit("Raw JSON extension endpoint is not wired.")
if "_smx_openai_create_video_via_json_endpoint(" not in func:
    raise SystemExit("Raw JSON image-reference create endpoint is not wired.")

runtime_file.write_text(content, encoding="utf-8")
print("Cleanly replaced _generate_openai_video with final raw JSON create/extend wiring.")
