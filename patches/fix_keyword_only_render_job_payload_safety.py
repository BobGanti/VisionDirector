...
# Ensure helpers exist before SQLiteRenderJobStore.
if "def _is_large_video_data_url(" not in content:
    marker = "class SQLiteRenderJobStore"
    idx = content.find(marker)
    if idx < 0:
        raise SystemExit("Could not find SQLiteRenderJobStore insertion point.")
...
start = content.find("    def mark_success(\n        self,\n        *,\n        job_id: str,")
if start < 0:
    raise SystemExit("Could not find actual keyword-only mark_success method.")
...
print("patched keyword-only SQLiteRenderJobStore.mark_success")
...
print("created tests/test_render_jobs_do_not_expose_video_payloads.py")
