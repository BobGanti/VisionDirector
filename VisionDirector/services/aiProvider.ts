...
const openaiProvider: AIProvider = {
  ...
  analyzeVoice: async (audioBase64, sentiment) => {
    try {
      // Prefer acoustic analysis from the raw waveform when a Google key is present.
      // This preserves accent / cadence far better than transcript-only inference.
      return await GeminiService.analyzeVoice(audioBase64, sentiment);
    } catch {
      return await OpenAIService.analyzeVoice(audioBase64, sentiment);
    }
  },
