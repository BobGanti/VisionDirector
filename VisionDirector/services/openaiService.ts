...
function mapVoiceToOpenAIVoice(v: VoiceProfile): string {
  ...
  const OPENAI_NATIVE = new Set([
    ...
    "verse", "ash", "sage", "ballad", "coral",
  ]);
  if (OPENAI_NATIVE.has(lower)) return lower;

  // Legacy mapping from your older Google-style “house voices”
  switch (raw) {
...
export class OpenAIService {
  ...
  static async generateVideo(
    ...
    const cleanText = (s?: string, maxLen: number = 1600) => {
      if (!s) return "";
      let t = String(s)
        .replace(/\r\n/g, "\n")
        .replace(/\r/g, "\n")
    ...
    const speakerLockBlock = startImageBase64 && cleanNarration
      ? [
          ...
          cleanTraits
            ? `- Preserve the uploaded reference voice DNA exactly: accent, pronunciation, cadence, pitch behaviour, rhythm, and vocal texture.`
            : `- Do not use an off-screen narrator or generic trailer voice.`,
          `- If the reference voice contains a regional accent such as Nigerian English, preserve it and do not neutralise it.`,
          `- Do not introduce another speaker.`
        ].join("\n")
      : "";

    const composedPrompt =
      `${speakerLockBlock ? `${speakerLockBlock}\n\n` : ""}` +
      `${cleanVisuals || ""}\n\n` +
      `Narration / voiceover:\n${cleanNarration || ""}\n\n` +
      `Voice: ${(prebuiltVoice || "alloy")}\n` +
      `Speed: ${(speed || "natural")}\n` +
      `Sentiment: ${(sentiment || "neutral")}\n` +
      (cleanTraits ? `Style traits:\n${cleanTraits}\n` : "");
    ...
    if (videoToExtend && typeof videoToExtend === "string") {
      const remixRes = await fetch(`${OPENAI_BASE}/videos/${videoToExtend}/remix`, {
        ...
        body: JSON.stringify({ prompt: composedPrompt, seconds, size }),
      });
