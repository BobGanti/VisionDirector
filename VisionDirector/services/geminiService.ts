...
export class GeminiService {
  ...
  static async generateVideo(
    ...
    //##################################
    const speakerLockBlock = cleanStart && cleanNarration
      ? [
          `[REFERENCE SPEAKER LOCK - HIGHEST PRIORITY]`,
          `- The supplied reference image is the exact on-screen speaker.`,
          `- The visible subject in the reference image must deliver the narration on camera.`,
          `- Keep facial identity, skin tone, hair, age, clothing, and framing faithful to the reference image.`,
          `- Animate natural mouth movement and facial performance that stays tightly synced to the narration.`,
          cleanTraits
            ? `- Preserve the uploaded reference voice DNA exactly: accent, pronunciation, cadence, pitch behaviour, rhythm, and vocal texture.`
            : `- Do not use an off-screen narrator or detached trailer voice.`,
    ...
    const finalPrompt = `${speakerLockBlock ? `${speakerLockBlock}\n\n` : ``}${visualOnly}\n\n${audioBlock}`;
