...
old_transcribe = '''        model = ModelRouter(
            profile_registry=profile_registry,
            overrides_store=_model_overrides_snapshot(),
        ).resolve(supplier, "DICTATION").model
'''

new_transcribe = '''        model = model_router.resolve(supplier, "DICTATION").model
'''

old_analyze = '''        router = ModelRouter(
            profile_registry=profile_registry,
            overrides_store=_model_overrides_snapshot(),
        )
...
new_analyze = '''        model = model_router.resolve(supplier, "VOICE_ANALYZER").model
        dictation_model = model_router.resolve(supplier, "DICTATION").model
'''

if old_transcribe not in content:
    raise SystemExit("Could not find transcribe ModelRouter block.")

if old_analyze not in content:
    raise SystemExit("Could not find analyze ModelRouter block.")

content = content.replace(old_transcribe, new_transcribe, 1)
content = content.replace(old_analyze, new_analyze, 1)

init_file.write_text(content, encoding="utf-8")
print("Replaced direct ModelRouter construction with existing model_router closure.")
