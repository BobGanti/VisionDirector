...
if "def _host_provider_status_payload():" not in content:
    ...
    content = content.replace(marker, status_helper + marker, 1)

credentials_status_route = '''
...
print("updated credential routes to be host-managed provider status routes")
...
if frontend_file.exists():
    ...
    js = re.sub(
        r"Paste your keys to use this deployment\\. Keys are encrypted and stored in the instance database\\.\\s*You can delete them any time\\.",
        "Credentials are managed by the SyntaxMatrix host. Model selection remains editable below. No browser API keys are required.",
        js,
        flags=re.MULTILINE,
    )

    js = re.sub(
        r"Paste your keys to use this deployment\\. Keys are encrypted and stored in the instance database\\.",
        "Credentials are managed by the SyntaxMatrix host.",
        js,
    )

    js = re.sub(
        r"You can delete them any time\\.",
        "No browser API keys are stored by this plugin.",
        js,
    )

    frontend_file.write_text(js, encoding="utf-8")
    print("updated frontend credential wording for host-managed plugin mode")
else:
...
print("Patch complete: credential UI/status layer is host-managed.")
