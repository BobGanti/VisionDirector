...
const Studio: React.FC<{ isBridgeMode?: boolean }> = ({ isBridgeMode = false }) => {
  ...
  const handleExecuteRender = async () => {
    try {
      ...
      let voiceTraits = "";
      // MANDATORY DNA PRIORITIZATION: If audio asset is selected, force analysis to ensure resemblance
      if (activeAudio) {
        setStatus(AppStatus.GENERATING_AUDIO);
        voiceTraits = await GeminiService.analyzeVoice(activeAudio.url, sentiment);
        // If narration is empty, default to transcription to maintain identity
        if (!script.narration) {
      ...
      setStatus(isExtensionMode ? AppStatus.GENERATING_VIDEO : AppStatus.GENERATING_VIDEO);
    ...
    } catch (e: any) { showError(e.message); } finally { setStatus(AppStatus.IDLE); }
  };
  ...
  const handleSaveIdentity = async () => {
    ...
    } catch (e) { showError("Cloning failed."); } finally { setStatus(AppStatus.IDLE); }
  };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newAsset: MediaAsset = { id: `asset-${Date.now()}`, type: type as any, url: `data:${file.type};base64,${base64}`, timestamp: Date.now(), fileName: file.name };
    setAssets(prev => [newAsset, ...prev]);
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="h-14 px-6 flex items-center justify-between border-b border-white/5 bg-black">
          ...
          <div className="flex items-center gap-4">
            <button onClick={() => setShowBlueprint(true)} className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-zinc-500 hover:text-white transition-all"><i className="fa-solid fa-share-nodes"></i></button>
          </div>
        </header>
        ...
        <div className="flex-1 bg-black flex flex-col items-center justify-center p-8 overflow-hidden relative">
           <div style={{ aspectRatio: aspectRatio.replace(':', '/') }} className="w-full max-w-4xl bg-[#0a0a0c] rounded-3xl shadow-2xl ring-1 ring-white/10 overflow-hidden flex items-center justify-center relative group">
             {activeAsset ? (
                activeAsset.type === 'video' ? <video src={activeAsset.url} className="w-full h-full object-contain" autoPlay loop controls /> : <img src={activeAsset.url} className="w-full h-full object-contain" />
             ) : (
             ...
             {isExtensionMode && (
               <div className="absolute top-6 left-6 flex flex-col gap-2">
                 <div className="bg-emerald-500 text-white text-[9px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest shadow-2xl animate-pulse">Director Protocol: Extension Mode</div>
                 <div className="bg-black/80 backdrop-blur-md border border-emerald-500/30 p-4 rounded-2xl max-w-xs animate-in slide-in-from-left-4 duration-500">
                    <p className="text-[8px] font-black text-emerald-400 uppercase tracking-widest mb-2 border-b border-emerald-500/20 pb-1">Operational Hints</p>
        ...
        <div className="p-8 border-t border-white/5 bg-[#080808]">
          <div className="max-w-6xl mx-auto">
            <div className="flex gap-6 mb-8">
              <div className="flex-1 space-y-3">
                <div className="flex justify-between items-center px-1">
                  <div className="flex items-center gap-3">
                    <span className="text-[9px] font-black uppercase tracking-widest text-zinc-600">{isExtensionMode ? "Extension Script" : "Sequence Narrative"}</span>
                    {activeAudio && (
                      <span className="text-[7px] font-black bg-emerald-500/20 text-emerald-400 px-3 py-1 rounded-full uppercase tracking-tighter border border-emerald-500/20">DNA Overriding Prebuilt Voice</span>
                    )}
                  </div>
                ...
                <textarea 
                  ...
                  className={`w-full h-24 bg-white/5 rounded-2xl p-6 text-sm font-medium border focus:border-violet-500/50 outline-none transition-all resize-none shadow-inner ${isExtensionMode ? 'border-emerald-500/30' : 'border-white/5'}`} 
              ...
              <div className="w-64 flex flex-col justify-end">
                <button 
                  ...
                  className={`w-full h-24 rounded-2xl flex flex-col items-center justify-center gap-2 transition-all shadow-2xl disabled:opacity-20 text-white font-black uppercase tracking-widest text-[10px] ${isExtensionMode ? 'bg-emerald-600 hover:bg-emerald-500 shadow-[0_10px_30px_rgba(16,185,129,0.3)]' : 'bg-violet-600 hover:bg-violet-500 shadow-[0_10px_30px_rgba(139,92,246,0.3)]'}`}
                >
            ...
            <div className="flex items-center justify-between border-t border-white/5 pt-6">
              ...
              <button onClick={handleSaveIdentity} className="px-6 py-2 rounded-xl border border-white/5 text-[9px] font-black uppercase text-zinc-500 hover:text-emerald-400 transition-all bg-white/[0.02] hover:bg-emerald-500/5 hover:border-emerald-500/20">Capture Identity</button>
            </div>
          </div>
        </div>
      </main>
